...
## subset data for Baltimore city : fips=="24510" and Los Angeles : fips= "06037"
NEI3 <- filter(NEI2, fips == "24510" | fips == "06037")
...
## creating a grouped data frame table by year by county
motor_year <- group_by(motor_emissions, fips, year)
...
## Plotting
library(ggplot2)
png("./course_asgn2/plot6.png")

p <- ggplot(motor_sum, aes(x=year, y= sum_year, group=fips))

## Adding line + points + title + clear background
p + geom_line(aes(color=fips)) + geom_point() + xlab("Year") + ylab("Total Motor Emissions (tons)") +
        ggtitle("Emissions from motor vehicle sources") + theme_bw()
